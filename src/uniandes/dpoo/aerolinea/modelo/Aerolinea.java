...
public class Aerolinea
{
    ...
    public void programarVuelo( String fecha, String codigoRuta, String nombreAvion ) throws Exception
    {
    	...
    	Ruta rutaActual = rutas.get(codigoRuta);
    	...
    	while (avion==null && count<aviones.size()) {
    		if (aviones.get(count).getNombre().equals(nombreAvion)) {
    			avion = aviones.get(count);
    		} count +=1;
    	}
    	
    	if (avion == null) {
    		throw new Exception("El avion indicado no existe");
    	}
    	
    	for(Vuelo vuelo: vuelos ) {
    		if (vuelo.getAvion().getNombre().equals(avion.getNombre())) {
    			if(vuelo.getFecha().equals(fecha)) {
    				Ruta rutaExistente = vuelo.getRuta();
    				int existenteFinHora = Ruta.getHoras(rutaExistente.getHoraLlegada());  
    				int existenteFinMinutos = Ruta.getMinutos(rutaExistente.getHoraLlegada());    				
    				int totalExistenteFin = (existenteFinHora*60) + existenteFinMinutos;
    				
    				int existenteInicioHora = Ruta.getHoras(rutaExistente.getHoraSalida());
    				int existenteInicioMinutos = Ruta.getMinutos(rutaExistente.getHoraSalida());
    				int totalExistenteInicio = (existenteInicioHora * 60) + existenteInicioMinutos;
    				
    				int nuevoFinHora =  Ruta.getHoras(rutaActual.getHoraLlegada());
    				int nuevoFinMinutos = Ruta.getMinutos(rutaActual.getHoraLlegada());
    				int totalNuevoFin = (nuevoFinHora *60) + nuevoFinMinutos;
    				
    				int nuevoInicioHora = Ruta.getHoras(rutaActual.getHoraSalida());
    				int nuevoInicioMinutos = Ruta.getMinutos(rutaActual.getHoraSalida());
    				int totalNuevoInicio = (nuevoInicioHora * 60) + nuevoInicioMinutos;
    				
    				if(totalNuevoInicio < totalExistenteFin && totalExistenteInicio < totalNuevoFin) {
    					throw new Exception("Cruce de horarios para el vuelo " + vuelo);
    				} 
    			}
    					
    		}
    	}
        Vuelo vuelo = new Vuelo(rutaActual, fecha, avion);
        vuelos.add(vuelo);
    ...
    public int venderTiquetes( String identificadorCliente, String fecha, String codigoRuta, int cantidad ) throws VueloSobrevendidoException, Exception
    {
    	...
    	if (cliente == null) {
    		throw new Exception("No se ha encontrado cliente con el identificador " + identificadorCliente);
    	}
    	
    	Vuelo vuelo = getVuelo(codigoRuta, fecha);
    	if (vuelo == null) {
    		throw new Exception("El vuelo con número de ruta " + codigoRuta + "y fecha " + fecha + "no existe");
    	}
    	
    	CalculadoraTarifas calculadora;
    	LocalDate fechaParse = LocalDate.parse(fecha);
    	int mes = fechaParse.getMonthValue();
    	...
    	if (cantidad<=0) {
    		throw new Exception("La cantidad de tiquetes debe ser mayor que 0");
    	}
